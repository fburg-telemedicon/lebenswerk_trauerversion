...
export async function addContribution({ memorialCode, contributorName, relationship, messages }) {
  const res = await fetch('/api/contributions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ memorialCode, contributorName, relationship, messages }),
  })
  const d = await res.json()
  if (!res.ok) throw new Error(d.error)
  return d // { id }
