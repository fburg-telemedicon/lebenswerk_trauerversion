...
const EMPTY_CREATE = {
  ...
  deleteOn: '',                     // manuell vereinbarter, spaeterer Loeschtag (leer = automatisches Datum)
...
function Dashboard() {
  ...
  // Wie refreshMemorials, aber zieht auch das gerade geoeffnete Buch nach.
  // refreshMemorials allein aktualisiert nur die Liste — die Detailansicht
  // arbeitet auf `selected` und saehe eine Aenderung sonst nicht.
  async function reloadMemorial() {
    ...
    try {
      ...
      const d = await res.json()
      setMemorials(d)
      const u = d.find(m => m.id === selected.id)
      if (u) setSelected(u)
    } catch { /* stilles Nachladen */ }
  }
  ...
  function startOrderEdit() {
    ...
    setOrderDraft({
      ...
      deleteOn: (m.delete_on || '').slice(0, 10),
...
- Gib AUSSCHLIESSLICH den übersetzten Text aus — keine Vorbemerkung, keine Erklärung, kein Markdown.`
      ...
      <div style={{ position:'fixed', inset:0, background:'rgba(28,25,23,.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100, padding:'1rem', overflowY:'auto' }}>
        <div style={{ ...S.card, maxWidth: 560, width:'100%' }}>
          ...
          <p style={{ ...S.muted, marginBottom:16 }}>
            ...
            Herunterladen daraus zusammengesetzt. Gelesen wird von Mia und Klaus, den natürlichsten
            Stimmen, die es zurzeit gibt.
          </p>
          <div style={{ display:'grid', gap:10, marginBottom:14 }}>
  ...
  if (view === 'create') return (
    <CreateView auth={auth} createForm={createForm} busy={busy} err={err} allowedSlugs={allowedSlugs} catalogs={catalogs} logout={logout} setView={setView} setCreateForm={setCreateForm} handleCreate={handleCreate} />
  )
