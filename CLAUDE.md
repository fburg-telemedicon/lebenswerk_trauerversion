...
- `src/precaution.js`, `src/precautionExport.js` — the **Vorsorgen-Mappe** of the `precaution` category: eight separately signable forms modelled on the Deutsche PalliativStiftung's *VORSORGEN! Mappe* (power of attorney, guardianship directive, advance healthcare directive, statement of values, funeral directive, palliative traffic light, plus blank sub-delegation and representative forms), preceded by a verification list and followed by a worksheet. Same shape as the lifework side products (prompt, JSON on `memorials.precaution`, deterministic renderer) with two differences that matter. It **fills names and tick boxes in** — the person dictated them expressly for this document, unlike the lifework folder, which infers them from a life story and must therefore name nobody. And every yes/no field is **three-valued**: `true`, `false` or `null` for "not decided", which prints as two empty boxes. Never collapse that to a boolean — a wrong "Nein" is a refusal the person never declared, and it would be followed in an emergency. The shared form toolkit `src/legalForms.js` carries the checkbox helpers (`checkbox`, `decision`, `choice`, `yesNo({value})`); they have defaults that leave the older documents unchanged.
...
There are **fourteen product categories**; `CATEGORY_ORDER` in `src/categories.js` is the authoritative list: `lifework`, `mamazone`, `anamnesis`, `anamnesis_kvsw`, `career`, `precaution`, `memorial`, `birthday`, `anniversary`, `farewell`, `service`, `company`, `newborn`, `encouragement`. Each memorial row carries `product_category`. Slugs also exist backend-side in `api/_lib/categories.js`, which additionally exports the predicates `isEnduserCategory` / `isAnamnesisCategory` / `isLifeworkCategory` / `isCareerCategory` / `isPrecautionCategory`.

Two of them produce **no book at all** and follow the same pattern (end-user category, fixed standard catalog, no images, structured JSON on the memorial row plus a deterministic renderer): `career` (Lebenslauf, spec in `PRODUKTKATEGORIE_LEBENSLAUF.md`) and `precaution` (Vorsorgevollmacht, spec in `PRODUKTKATEGORIE_VORSORGE.md`). Copy that pattern for the next category of that shape; `anamnesis` is the older, section-based variant of it.
...
**Geschichtsbuch-Funktion** (2026-09-15): `memorials.history_mode` (boolean, default off, lifework family only) makes `interviewSystemFor` append `historyDirective` — the interview then asks *when* each episode happened. Like `detailDirective`, it is appended **centrally** so none of the six call sites in the contributor flow can forget it, and `api/memorial.js` must deliver the column or the setting never reaches the prompt. The companion products are `memorials.history_parallels` (jsonb; the dated events plus what happened then, prompt in `src/historyParallels.js`). The dates come from the **interviews**, not the book (since 2026-09-22): the book is a different text after every regeneration and silently lost its years. The book is only used to assign each date to a chapter. With `history_mode` on, `historyBookRule` also makes the chapter prompt carry every dated mention into the text and `POST /api/admin/history-box`, which writes an explanatory box into a chapter via the existing `chapter.boxes` mechanism — so print PDF, e-book, audiobook and the on-screen view render it without changes. Boxes may carry `image_path`; `collectImagePaths`/`applySignedUrls` walk `chapter.boxes` for that reason. Both prompts are deliberately conservative about facts: omit rather than guess, and every event carries a confidence flag the UI surfaces as "ungeprüft".
