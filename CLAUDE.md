...
- **Admin panel** — no URL flag; the user logs in via `/api/admin/login`, the bearer token is stored in `sessionStorage` as `lw_admin_token` (plus `lw_admin_auth` = `{ admin, cats }`). Views: `list` → `create-category` → `create` → `created`; `detail` (one memorial) → `contribution`, `book-v1`, `book-v2`, `eulogy`, `costs`; `users` (admin-only group/user management). The detail view also embeds a cost-breakdown table fetched in parallel with contributions.

### Product categories & customer groups (multi-tenant)

There are **six product categories** (slugs in `api/_lib/categories.js` and `src/categories.js`): `memorial` (Gedenkbuch — the original behaviour), `birthday`, `anniversary`, `farewell`, `newborn`, `encouragement`. Each memorial row carries `product_category`.
...
Auth is now **multi-user**. Login (`api/admin/login.js`) accepts either the env superadmin (`ADMIN_USERNAME`) → token claims `{ admin:true, cats:'*' }`, or a row in `app_users` (scrypt-hashed password) belonging to a `customer_groups` row → claims `{ uid, admin:false, cats: group.allowed_categories, group }`. `checkAuth` (in `api/_lib/auth.js`) verifies the signed token and attaches `req.auth`; `canAccessCategory(req.auth, cat)` gates creation. `api/admin/users.js` (admin-only) is the CRUD for groups/users. Non-admins only see memorials where `owner_group` = their group AND `product_category` ∈ their cats (filtered in `api/admin/memorials.js` GET). Memorial creation now goes through the **authenticated** `POST /api/admin/memorials` (not the public `POST /api/memorial`), so `owner_group`/`product_category` are set server-side from the token.
...
- `memorials.product_category` (text, default `'memorial'`), `memorials.owner_group` (uuid → `customer_groups`), `memorials.intake` (jsonb, category-specific optional fields) — added by `supabase/users.sql`
- `customer_groups` (`id`, `name`, `allowed_categories` text[], `created_at`) and `app_users` (`id`, `username`, `pw_hash`, `pw_salt`, `group_id`, `is_admin`, `created_at`) — defined in `supabase/users.sql`
