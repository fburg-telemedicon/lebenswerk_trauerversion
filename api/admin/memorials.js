  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const insertRow = {
        ...
        // Echtes Sprachgespräch (Realtime/Voice Live) als zusätzlicher Interview-Modus
        // (alle Interview-Kategorien). Default AUS — erst nach der DSGVO-Freigabe
        // (EU-only-Bestätigung) und nur für abgedeckte Sprachen wirksam; die Mikro-Modi
        // bleiben immer als Fallback. Siehe api/voicelive-token.js + src/voicelive.js.
      ...
      if (error && /image_style|book_layout|show_contributors|text_style|interview_timer_seconds|companion_mode|proof_enabled|proof_max|proof_used|show_onboarding|tts_voice|gamification|hands_free|mic_manual_stop|mic_mode_switch|guest_enabled|guest_code|realtime_enabled|column/i.test(error.message || '')) {
    ...
    if (req.method === 'PATCH') {
      ...
      if (meta && typeof meta === 'object') {
        ...
        if (error && /image_style|book_layout|text_style|interview_timer_seconds|companion_mode|show_contributors|proof_enabled|proof_max|proof_used|edit_lock|show_onboarding|tts_voice|gamification|hands_free|mic_manual_stop|mic_mode_switch|guest_enabled|guest_code|realtime_enabled|column/i.test(error.message || '')) {
