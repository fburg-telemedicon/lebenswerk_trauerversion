...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (error && /show_contributors|family_tree|life_poster|care_directive|power_of_attorney|archived_at|cv|avoca|documents|column/i.test(error.message || '')) {
    ...
    if (req.method === 'PATCH') {
      ...
      const allowedFields = new Set(['book_v1', 'book_v2', 'eulogy_text', 'content_reports', 'family_tree', 'life_poster', 'care_directive', 'power_of_attorney', 'cv', 'avoca', 'documents'])
      if (!allowedFields.has(field)) {
        return res.status(400).json({ error: 'Ungültiges Feld.' })
      }
      if (field === 'family_tree' || field === 'life_poster' || field === 'care_directive' || field === 'power_of_attorney' || field === 'cv' || field === 'avoca' || field === 'documents') await ensureLifeworkSchema()
