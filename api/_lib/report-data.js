...
async function gatherReport(supabase, opts = {}) {
  ...
  let memorials = await safe('memorials(full)', async () => {
    const { data, error } = await supabase.from('memorials')
      .select('id, product_category, created_at, book_v1_at, book_v2_at, eulogy_at, funeral_date, delete_on, uploaded_images, languages, purge_info')
    if (error) throw error
    return data || []
  }, null)
  ...
  if (memorials === null) {
    ...
    memorials = await safe('memorials(minimal)', async () => {
      const { data, error } = await supabase.from('memorials')
        .select('id, product_category, created_at, funeral_date, delete_on, uploaded_images, languages, purge_info')
      if (error) throw error
      return data || []
    }, [])
  }
